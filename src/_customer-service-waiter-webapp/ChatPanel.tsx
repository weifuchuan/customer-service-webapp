...
import { StoreContext, ImContext, useBus } from './store';
import {
  AccountBaseInfo,
  IMessageDetailModel,
  IRemindPushPayload
...
import { toJS, autorun, runInAction } from 'mobx';
...
const ChatPanel = observer(() => {
  ...
  const send = useCallback(async (content: string) => {
    im.sendChatMsg(store.currentContact!.id, 1, content);
    return true;
  }, []);

  const roomKey = store.currentContact
    ? im.store.otherIdToRoomKey.get(store.currentContact.id)
    : undefined;

  const messageList: IMessageDetailModel[] =
    store.currentContact && roomKey
      ? im.store.roomKeyToMessageList.has(roomKey)
        ? im.store.roomKeyToMessageList.get(roomKey)!.slice()
        : []
      : [];
  ...
  const getMore = useCallback(async () => {
    if (store.currentContact && !state.fetching && state.hasMore) {
      state.fetching = true;
      let roomKey = im.store.otherIdToRoomKey.get(store.currentContact.id);
      if (!roomKey) {
        const room = await im.fetchRoomInfo(1, store.currentContact!.id);
        roomKey = room.roomKey;
      }
      const hasMore = await im.nextMessagePage(roomKey);
      runInAction(() => {
  ...
  const goBottomListener = {
    on: (f: Function) => {
      const listener = (remind: IRemindPushPayload) => {
        console.log(remind.roomKey, roomKey);
        if (remind.roomKey === roomKey) {
          f();
        }
      };
      im.bus.addListener('remind', listener);
      ...
      return () => {
        ...
        im.bus.removeListener('remind', listener);
      };
    }
  };
  ...
  useEffect(
    () => {
      (async () => {
        if (store.currentContact) {
          let roomKey: string;
          if (im.store.otherIdToRoomKey.has(store.currentContact.id)) {
            roomKey = im.store.otherIdToRoomKey.get(store.currentContact.id)!;
          } else {
            roomKey = (await im.fetchRoomInfo(1, store.currentContact.id))
              .roomKey;
          }
          im.clearRemind(roomKey);
        }
      })();
    },
    [ store.currentContact ]
    ...
    <_ChatPanel>
      ...
      <div>
        <div>
          <div>
            <MessageList
              me={im.store.me || ({} as AccountBaseInfo)}
